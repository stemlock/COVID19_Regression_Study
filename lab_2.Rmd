...
model2b <- lm(case_rate_100k ~log(population_density) + I((white_pct)^2) , data = df)
coeftest(model2b, vcov = vcovHC)

```

For Model 3. Policy related variables will be introduced for the analytics.
...
hist(df$case_rate_100k[df$mask_legal == 1], xlab='Case Rate', ylab='Count', main='Case Rate - Mask Legal')
hist(df$case_rate_100k[df$mask_legal == 0], xlab='Case Rate', ylab='Count', main='Dem Voter - Mask not legal')

hist(df$case_rate_100k[df$mask_public_bool == 1], xlab='Case Rate', ylab='Count', main='Case Rate - Mask Mandate in Public')
hist(df$case_rate_100k[df$mask_public_bool == 0], xlab='Case Rate', ylab='Count', main='Dem Voter - No Mask Mandate in Public')

model3 <- lm(case_rate_100k ~log(population_density) + I((white_pct)^2) + mask_legal, data = df)
model3 <- lm(case_rate_100k ~ log(population_density) +white_pct+ mask_public_bool, data = df)
#model3 <- lm(death_rate_100k ~ log(population_density) + mask_public_bool, data = df)
