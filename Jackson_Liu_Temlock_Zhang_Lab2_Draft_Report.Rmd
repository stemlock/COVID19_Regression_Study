...
In order to allow for more simplistic and direct interpretations when analyzing the effect of the input variables on the output, it was determined that all percentage variables (senior, poverty, and unemployment) would be transformed into rates per 100,000 to align with the case rate variable. It will also serve to ease the burden on the readers of this report as like-for-like scaled relationships can be drawn between the variables. Given that this transformation is constant for all values (scalar multiplication of 100,000 for the seniors variable, and 1,000 for poverty and unemployment), it will have no effect on the distributions of the variables.
...
df <-df %>%
  mutate(    
    ...
    poverty_per_100k = 100000*poverty_pct/100, 
    unemployed_per_100k = 100000*unemployed_pct/100,
  )
```
...
```
As shown in the above distribution, there is a fairly normal distribution in the poverty number 
per 100,000 across the states. Additionally, there is no outliers observed within the distribution,
given that the range of rates fall between 7,600 and 19,700. Therefore, there is no further transformation in need for this variable in the following analysis process for both model 2 and model 3.
...
```
According to the histogram above, the distribution of unemployed number per 100,000 is of fairly normal distribution. Overall, it is not highly skewed or heavily tailed. Therefore, there is no further transformation in need for this variable in the following analysis process for both model 2 and model 3.
...
case_poverty <- df %>% 
  ...
  geom_smooth(se=FALSE)+
  labs(
    title = ' Association between Covid-19 ase rate and the poverty rate', 
    x = 'Number of living under poverty line per 100,000',
    y = 'Covid-19 care rate per 100,000', 
...
case_unemployed <- df %>% 
  ...
  geom_smooth(se=FALSE)+
  labs(
    title = 'Association between Covid-19 ase rate and the unemployment rate', 
...
```
However, there is an overall increasing trend of COVID-19 care rate with increasing poverty rate, while there is a inverse relationship between the variables of the COVID-19 case rate and the unemployment rate. 

Based on above data exploratory result, the following equation is used to create a regression model
to determine the relationship between the case rate and demographic features including the seniors rate, the rate of poverty and the rate of unemployment. 
...
Accordingt to the assessment of homoskedasticity of model 2, which would be discussed in the following paragraph (Model 2 limits - iv.Homoskedastic Errors), it is plausible to apply the classical standard errors in the t-test of coefficients here. 

```{r}
coeftest(model2)
```

Based on the test results of the estimated coefficients in model 2, all three demographic features discussed here are significantly realated to the COVID-19 care rate, especially for the poverty rate (p-value < 0.01).

The coefficient of the senior_per_100k is  -0.230970 and statistically significant, which implies that for every one thousand additional seniors per 100,000, there is a decrease of 231 COVID-19 cases per 100,000 holding other demographic features as constant. As discussed in mode1 1 section, although seniors are susceptible group, less mobility and social interaction with cautious self procetion may play a important role in this pandemic situation. 

In addition, the coefficient of the poverty_per_100k (0.231750) indicats that for every one thousand additional people living under poverty line per 100,000, there is a increase of 232 COVID-19 case per 100,000 holding other demographic features as constant. People living under the poverty line have limited accesss to medical resourses. Moreover, most of them could only afford to live in the communites with poor hygiene.  condition. 

Finally, the coefficient of the unemployed_per_100k is statiscally significant and negative(-0.486191), which implies that for every one thousand additional unemployed, there is a decrease of 486 COVID-19 case per 100,000 holding other demographic features as constant. Although the unemployment leads to less medical insurace coverage and impact on the incoming, the unempolyed people may have more flexibility in obeying stay-at-home. In addition, there is less expourse probability when no commuting or in-place working is required for the unemployed. 

#### Model 2 limits

##### i. IID Sampling 
(Would be discussed in the following section - 3. Limitations of the Models)

##### ii.Linear Conditional Expectation
To assess the linear conditional expectation assumption in higher-dimensional space, the plots of predicted versus residuals of the model 2 is investigated as follow.  
...
Overall, the residuals maintain around zero across the predicted range. There is no assumption violation phenomenon could be observed here. 

#####iii.No Perfect Collinearity

Since there are multiple covariates included in this model compared to the model 1, there is more concern about the no perfect collinearity assumption here
First, the list of variables coefficients shows that no variables were dropped, which mean there is no perfect collinearity detected by the R function. 

```{r}
model2$coefficients
```

In addition, all the variance inflation factors are less than 4 as follow, which doesn't indicate the exisitence of collinearity. 
...
To evaluate the homoskedastic errors assumption, the square root of the residuals is plotted against the fitted values of the model 2 (scale-location plot). Although, the curve is not perfect flat smooth, there is no obvious variance of the errors either. For reference, the data points marked in the plot represent Florida(10), New York (33) and North Dakota (35).

```{r, echo=FALSE}
plot(model2, which=3)
```

Additionally, a Breusch-Pagan test is run to check the level of heteroskedasticity. The test result shows it fails to reject the null hypotheis, which means there is no evidence for heteroskedasticity. Overall, the assumption of homoskedastic errors is satisfied for model 2. 
...
To access the assumption of normality of error distribution, both the histogram and the qqplot of the residuals is shown as follow.
...
plot_one / plot_two
```
Limited by the sample size, it is hard to observe a perfect normal distribution from the histogram. However, the qqplot implies no deviation from normality. Overall, the normally distributed errors assumption is satisfied for model 2. 

##### Cook Distance 

Additionally, cook distance is used to estimate the influence of each single data point. 

```{r, echo =FALSE}
plot(model2, which = 5)
```
No obvious outlier could be detected here. For reference, the marked data points are Florida(10), Maine(20), and North Dakota(30).
...
Next, the regression model for model 3 is based on the existing work from model 2. Two additional variables discussed in Model 3 will be introduced as maximalist approach. The first variable is the "mask_public_bool" (whether the state enforced public mask mandate or not) that represents the policy effects. The second variable introduced is "population_density" (the value is logged as discussed earlier) which is considered an important factor considering nature of spread in disease. 
...
#### Normality of errors
From the Normal Q-Q plot below, the points generally stay close to the normal line, with the tails exhibit certain degree of skew. 
```{r}
plot(model3, which=2)
```
