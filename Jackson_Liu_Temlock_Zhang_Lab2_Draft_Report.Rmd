...
```{r}
model2a <- lm(case_rate_100k ~ senior_per_100k + log(population_density) + noneld_pre_existing_per_100k + poverty_per_100k + white_per_100k, data = df)
coeftest(model2a)
```
...
$$
```{r}
model2b <- lm(case_rate_100k ~ senior_per_100k + log(population_density) + noneld_pre_existing_per_100k + poverty_per_100k + white_per_100k + unemployed_per_100k, data = df)
coeftest(model2b)
```
...
Firstly, "mask_public_bool" column is created from "mask_public", with 1 represents that the state has a pubic mask policy, and 0 means there's no explicit public mask policy from the state.
...
```{r}
model3a <- lm(case_rate_100k ~ senior_per_100k + log(population_density) 
              + noneld_pre_existing_per_100k + poverty_per_100k + white_per_100k 
              + unemployed_per_100k + mask_public_bool, data = df)
...
```{r}
df %>% {
   ggplot(mapping = aes(x = log(df$population_density), y = model3a$residuals)) +
...
df %>% {
   ggplot(mapping = aes(x = df$poverty_pct, y = model3a$residuals)) +
      geom_point() + stat_smooth(se = TRUE) +
...
df %>% {
   ggplot(mapping = aes(x = log(df$mask_public_bool), y = model3a$residuals)) +
      geom_point() + stat_smooth(se = TRUE) +
...
```{r}
hist(model3a$residuals, main = "Model Residuals", xlab = "Residual")
```
...
df<- df%>% 
  mutate(
    model2a_preds = predict(model2a), 
    model2a_resids = resid(model2a)
  ) 
df %>% 
  ggplot(aes(model2a_preds, model2a_resids)) + 
  geom_point() + 
  stat_smooth()
```
...
```{r, echo=FALSE}
plot_one <- df %>% 
  ggplot(aes(x = model2a_resids)) + 
  stat_bin(bins = 25)
  geom_histogram()
  
plot_two <- df %>% 
  ggplot(aes(sample = model2a_resids)) + 
...
df<- df%>% 
  mutate(
    model2b_preds = predict(model2b), 
    model2b_resids = resid(model2b)
  ) 
df %>% 
  ggplot(aes(model2b_preds, model2b_resids)) + 
  geom_point() + 
  stat_smooth()
```
...
```{r, echo=FALSE}
plot_one <- df %>% 
  ggplot(aes(x = model2b_resids)) + 
  stat_bin(bins = 25)
  geom_histogram()
  
plot_two <- df %>% 
  ggplot(aes(sample = model2b_resids)) + 
  stat_qq() + stat_qq_line()
...
This variable could also be partially explained by the senior percentage variable, because of the gap in gender ratios across generations and generally as people get older. Because men have lower life expectancies, older groups of people living in the US would have higher ratios of women to men. Therefore, introducing the gender ratio variable into the model would lower the value of the coefficient to the senior percentage variable. This would be a positive omitted variable bias away from zero. 
...
A variable capturing the mask use among the population would likely reduce the explanatory power of the state mask mandate variable. This would cause the bias of the omitted variable to be positive and away from zero. 

# 5-5: GDP Percentage of Tourism

The economic standing of a state not only affects the individual households of its population, but also includes how it interacts with other states and how its peoples' lives might have been economically changed by the pandemic. One way that a state may increase its number of covid cases is both from tourists or others visiting during the pandemic, but also if its own population must work in close contact with others. In addition, if certain key industries to a state, such as tourism, layed off many workers, they would likely have to take lower paying close contact jobs such as in the service or gig economy in order to continue having a livelihood. 

This effect can be included with a variable that demonstrates how much of a state's GDP is from tourism. It would be related to other economic indicator variables, such as the percent of the population living in poverty. While poverty is a broader phenomena, the number of people in poverty in the pandemic when the data was taken would be potentially higher due to negative effects in the tourism industry. Therefore, it is expected that states with a greater focus on tourism may also have more people in poverty in the pandemic. This would take some of the explanatory power of the poverty variable away, resulting in a positive omitted variable bias away from zero. 
