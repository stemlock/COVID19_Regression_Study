...
df<-df%>%
  ...
  select(case_rate_100k, death_rate_100k,population_density, white_pct, black_pct, hispanic_pct, 
         other_pct, state_emergency, business_closed, business_reopen, mask_public, mask_legal, Black...of.Total.Population, White...of.Total.Population, Percent.living.under.the.federal.poverty.line..2018., Percent.Unemployed..2018., X65.)
...
### Model 1 & Model 2 Building Process
...
hist_log_population_dens <- ggplot(data = df, aes(x = log(population_density))) + 
...
```{r}
model1a <- lm(case_rate_100k ~ population_density, data = df)
...
$$
```{r}
model1b <- lm(case_rate_100k ~ log(population_density) , data = df)
...
```{r}
model2b <- lm(case_rate_100k ~log(population_density)+ White...of.Total.Population+Percent.living.under.the.federal.poverty.line..2018., data = df)
...
$$
```{r}
model2b <- lm(case_rate_100k ~log(population_density)+ I((White...of.Total.Population)^2) + Percent.living.under.the.federal.poverty.line..2018., data = df)
coeftest(model2b, vcov = vcovHC)
```
$$
\begin{aligned}
...
$$
```{r}
model2b <- lm(case_rate_100k ~ Percent.living.under.the.federal.poverty.line..2018., data = df)
coeftest(model2b, vcov = vcovHC)
```
